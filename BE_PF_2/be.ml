...
(* evalue 
   ...
   x : valeur pour laquelle on veut évaluer le polynome
   Retour : Valeur du polynôme évaluer*)

let evalue poly x= ((List.hd poly) +. (List.fold_left (fun acc e -> e*.x+.acc) 0. (List.tl poly)))
...
let rec retract = fun liste -> match liste with
| hd::tl when (evalue tl 1.)=0. -> []
| hd::tl -> retract tl
| [] -> []
...
let scal_mult = fun liste lambda -> List.map (fun a -> a*.lambda) liste
...
let rec plus  = fun pol1 pol2 -> match pol1,pol2 with
  | [],[] -> []
  | hd1::tl1, hd2::tl2 -> [hd1+.hd2]:: (plus tl1 tl2)
  | [], hd2::tl2 -> [hd2]::(plus [] tl2)
  | hd1::tl1, [] -> [hd1]::(plus tl1 [])
